# SMS Messenger - Frontend Design

**Frontend-specific design and implementation requirements. This document is independent of backend implementation details.**

## Overview

Frontend implementation for the SMS Messenger utility. Provides a chat-style interface for sending and receiving SMS messages, managing contacts, and viewing conversation threads.
...
See `docs/features/sms-messenger/API_CONTRACT.md` for the API contract this frontend consumes.
...
**Description**: Main page for SMS messaging utility
...
### SMSMessenger (Main Page)

**Location**: `SMSMessenger.tsx`

**Purpose**: Main page component that renders the SMS interface

**State**:

-   Server state: Initial threads, messages, contacts (TanStack Query)
-   Local state: None (delegates to SMSInterface)

### SMSInterface

**Location**: `components/SMSInterface/SMSInterface.tsx`
...
-   CSS Modules: `SMSInterface.module.css`
...
const queryKeys = {
    ...
    messages: (counterpart: string) =>
        ["sms-messenger", "messages", counterpart] as const,
