...
**NZBGet Configuration**: The container configures NZBGet in two ways:

1. **Config file** (`-c` flag): Sets directory paths at startup (where downloads go).
2. **API calls**: Sets Usenet server credentials and enables RAR unpacking at runtime. Unpacking is critical because many Usenet releases are password-protected RAR archives - NZBGet extracts the embedded password from the NZB metadata and unpacks automatically.
